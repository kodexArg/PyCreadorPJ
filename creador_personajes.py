...
def randomizar_habilidades(caracter, atributos):
    ...
    df_habilidades = pd.DataFrame(
        hlist, columns=["HABILIDAD", "ATR", "INI", "WIG", "VALOR"]
    )
    df_habilidades = (
        df_habilidades[["HABILIDAD", "VALOR"]]
        .set_index("HABILIDAD")
...
def randomizar_inventario(caracter, habilidades, atributos, caracteristicas):
    ...
    for inv in inventario:
        if inv in hab_pj:
            ...
            for item, atr in inv_posible.items():
...
def randomizar_dinero(caracter):
    cash = np.random.randint(0, np.int32(caracter["poder"])) * 10
    return {"dinero": cash}
...
def randomizar_nombre(caracter):
    ...
    if caracter["opt_nombre"]:
        ...
        while pruebas < ct.INTENTOS:
            ...
            chances_aceptar = np.float32(test["frec"].values[0] / nombres_max)
    ...
    if caracter["opt_apellido"]:
        ...
        while pruebas < ct.INTENTOS:
            ...
            chances_aceptar = np.float32(test["frec_pri"].values[0] / nombres_max)
            if np.random.random() < chances_aceptar:
                break
